...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const arrow = btn.querySelector('svg .d');
  const progressPath = btn.querySelector('svg .scrollProgress');

  // 确保初始是“不可见”状态（用类控制，而不是 display）
  btn.classList.remove('vbl');
  ...
  if (progressPath) {
    ...
    Object.assign(progressPath.style, {
      transition: 'none',
  ...
  const updateVisibility = () => {
    ...
    const visible = getScrollTop() > 0;
    // 首次需要显示时，去掉 hidden，交给 CSS 动画
    btn.hidden = !visible;
    btn.classList.toggle('vbl', visible);
    if (arrow) arrow.style.visibility = visible ? 'visible' : 'hidden';
  };
  ...
  const updateProgress = () => {
    ...
    progressPath.style.strokeDashoffset = pathLength - (scrollTop * pathLength / height);
  };
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (saved === "true") {
    ...
    // 清理变量痕迹
    Object.defineProperty(window, "payload", { value: null });
